    ...
    !function(t, e) {
      ...
      class fs extends W {
        ...
        _keydown(t2) {
          ...
          i2 && (i2.focus({ preventScroll: true }), fs.getOrCreateInstance(i2).show());
        }
        _getChildren() {
        ...
        _toggleDropDown(t2, e2) {
          ...
          s2(".dropdown-toggle", ls), s2(".dropdown-menu", hs), i2.setAttribute("aria-expanded", e2);
        }
        ...
        _getInnerElement(t2) {
          return t2.matches(_s) ? t2 : K.findOne(_s, t2);
        }
        _getOuterElement(t2) {
        ...
        static jQueryInterface(t2) {
          return this.each(function() {
            const e2 = fs.getOrCreateInstance(this);
            if ("string" == typeof t2) {
              if (void 0 === e2[t2] || t2.startsWith("_") || "constructor" === t2)
                throw new TypeError(`No method named "${t2}"`);
              e2[t2]();
      ...
      j.on(document, Zi, us, function(t2) {
        ["A", "AREA"].includes(this.tagName) && t2.preventDefault(), d(this) || fs.getOrCreateInstance(this).show();
      }), j.on(window, es, () => {
        for (const t2 of K.find(gs))
          fs.getOrCreateInstance(t2);
      }), b(fs);
      const ms = ".bs.toast", ps = `mouseover${ms}`, bs = `mouseout${ms}`, vs = `focusin${ms}`, ys = `focusout${ms}`, ws = `hide${ms}`, As = `hidden${ms}`, Es = `show${ms}`, Cs = `shown${ms}`, Ts = "hide", ks = "show", $s = "showing", Ss = { animation: "boolean", autohide: "boolean", delay: "number" }, Ls = { animation: true, autohide: true, delay: 5e3 };
      class Os extends W {
        constructor(t2, e2) {
        ...
        _setListeners() {
          j.on(this._element, ps, (t2) => this._onInteraction(t2, true)), j.on(this._element, bs, (t2) => this._onInteraction(t2, false)), j.on(this._element, vs, (t2) => this._onInteraction(t2, true)), j.on(this._element, ys, (t2) => this._onInteraction(t2, false));
        }
        _clearTimeout() {
        ...
        static jQueryInterface(t2) {
          return this.each(function() {
            const e2 = Os.getOrCreateInstance(this, t2);
            if ("string" == typeof t2) {
              if (void 0 === e2[t2])
                throw new TypeError(`No method named "${t2}"`);
              e2[t2](this);
      ...
      return V(Os), b(Os), { Alert: U, Button: J, Carousel: Ot, Collapse: Rt, Dropdown: fe, Modal: Ue, Offcanvas: gi, Popover: Mi, ScrollSpy: Qi, Tab: fs, Toast: Os, Tooltip: Ni };
    });
  }
});
